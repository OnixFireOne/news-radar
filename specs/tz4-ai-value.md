...
| 13.09 | v1.8 | Решения владельца после разметки golden set: (1) р.3.1 — главный критерий профиля «применимость для того, кто пользуется готовыми ИИ» важнее измеримого результата, инструменты по ИИ могут получать 8–10; (2) р.3.2/8.1 — крипта на паузе: квота `crypto` = 0, телеграм-коллектор выключается ключом `sources.telegram.enabled`; (3) р.6/8.2 — починка облачного режима (`LLM_API_KEY` не читался) — обязательный первый шаг И3; (4) р.9 — критерии по golden set привязаны к `tests/golden/` |
...
- `value_score`: 8–10 — конкретный кейс с измеримым результатом/уроком **либо инструмент/статья, прямо помогающие эффективнее работать с готовыми ИИ**; 4–7 — полезный туториал/инструмент/релиз с деталями; 1–3 — мнение без вывода, хайп, анонс без деталей.
- **Главный критерий профиля (v1.8, решение владельца по golden set).** Выше всего — **применимость для того, кто пользуется существующими ИИ**: как работать с ними эффективнее, как их встраивать, как не наступить на грабли. Измеримый результат сам по себе оценку не поднимает: материал про обучение своих моделей или классический ML с цифрами — это 5–7, а не 8–10 (примеры `g01`, `g19`, `g23`, `g34` в `tests/golden/LABELING.md`). Инструменты и релизы **по теме ИИ** оцениваются наравне с кейсами и могут получать 8 (пример `g04`).
- **Разметка датирована.** `tests/golden/` фиксирует приоритеты владельца на 11.09.2026. Когда фокус сменится — перерамечается набор, а не подгоняется классификатор. Систематическое расхождение классификатора с разметкой сначала проверяется вопросом «не устарела ли разметка».
...
| crypto | **0 — крипта на паузе (v1.8)**; механизм остаётся, включается возвратом квоты в конфиге |
...
- **⚠️ Облачный режим сейчас не подключится (найдено 11.09, чинить первым делом в И3).** `LLMClient.__init__` имеет `api_key: str = "not-needed"`, и `self.api_key = api_key or os.getenv("LLM_API_KEY", ...)` никогда не доходит до env: вызовы `LLMClient(timeout=300)` в `analyzer/analyzer.py` и `api/main.py` подставляют заглушку. Починить (дефолт `None`), добавить тест на то, что ключ берётся из `LLM_API_KEY`, и **не логировать сам ключ** (`llm_core.mask`). Ключ `llm_model` в `settings.json` никто не читает — модель берётся из env; либо читать его, либо убрать из примеров, чтобы не вводил в заблуждение.
- **Прокси:** aiprimetech.io из morning-post (`/v1/chat/completions`). Учёт входных токенов по нему врёт — `prompt_tokens: 0`; в отчёте о расходах это отметить, а не считать за правду.
- Решение владельца (13.09): **режим — облачный** (`llm_local_mode: false`), ключ вписывает владелец в реальный `.env`; новых переменных не заводим, хватает `LLM_BASE_URL` / `LLM_API_KEY` / `LLM_MODEL` из `.env.example`.
...
Переменные уже есть в `.env.example`, новых заводить не нужно — владелец вписывает значения в реальный `.env`:

```bash
LLM_BASE_URL=https://<твой-прокси>/v1
LLM_API_KEY=...        # реальный ключ прокси; до v1.8 не читался — см. р.6
...
- [ ]  Golden set: `tests/golden/LABELING.md` + `tests/golden/value_candidates.jsonl` (34 размеченных примера, разметка владельца 11.09.2026 — ценно 18, шум 11, хайп 5). Классификатор отделяет «ценно» от «хайп/шум» с точностью ≥ 80%; хайп никогда не получает `value_score ≥ 8`. Отдельно смотреть состязательные примеры: `g28` (длинный SEO-лонгрид), `g24` (подборка с громкой статистикой), `g21` (полезный инструмент не по теме), `g30` (запись из одного заголовка), `g20`/`g31` (текст с инструкциями внутри).
- [ ]  Крипта на паузе: при `quotas.crypto = 0` и `sources.telegram.enabled = false` дайджест собирается без крипто-материалов, и ни один старый механизм при этом не удалён — возвращается правкой конфига.
...
3. **И3 — Мозг:** профиль `ai_value` (классификация), golden set + evals, воронка с квотами. Порядок работ внутри И3 (уточнено 13.09):
    1. чистка сырого HTML в `messages.text` из RSS (сейчас Хабр/dev.to/Simon приходят с `<img>`/`<p>`), порог «короткий сниппет → догрузить» мерить по тексту, а не по HTML;
    2. починка облачного режима (р.6) — без неё классификатор не запустить на прокси;
    3. крипта на паузе: `sources.telegram.enabled = false`, `quotas.crypto = 0`;
    4. промпт и схема профиля `ai_value` (р.3.1), затем `tests/eval_value_scoring.py` на `tests/golden/`;
    5. воронка с квотами (р.3.2).
