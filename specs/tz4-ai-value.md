...
- Расширить допустимые `source_type`: `rss`, `hackernews`, `github`, `reddit` (в `RawMessage` ничего менять не нужно — поля уже с дефолтами).
...
- В `analyzer/llm_client.py` — фич-флаг `llm_local_mode` (дефолт `false` в новом профиле):
...
| `collectors/base.py` | не менять интерфейс; при необходимости — только docstring про новые `source_type` |
