...
| И3 | Мозг: профиль `ai_value`, golden set + evals, воронка с квотами | ⏸ после приёмки И2 | — |
...
- [ ] **СЛЕДУЮЩЕЕ ДЕЙСТВИЕ: приёмка И2.** Сценарий целиком — в `specs/reports/tz4-i2-acceptance.md`. Тестирования не было вообще: код исполнитель сдал, владелец не проверял.
- [ ] **Живая проверка коллекторов** — фиды в `config/settings.json` уже заполнены; нужно только выставить `sources.*.enabled = true` и поднять `docker compose --profile feeds up -d collector-feeds`.
